...
class H2OFrame(object):
    ...
    def drop(self, index, axis=1):
        """
        Drop a single column or row or a set of columns or rows from the current H2OFrame.
        ...
        if axis == 1:
            ...
            if is_type(index, str):
                ...
                assert set([index]).issubset(self.names) == True, "Column(s) selected to drop are not in original frame: %r" % index
            ...
            #If input is an int array indicating a column index, i.e., [3] or [1,2,3]:
            elif all(isinstance(item, int) for item in index):
                ...
                for i in range(len(index)):
                        index_to_drop[i] = index_to_drop[i] + 1
                index_to_drop = [-x for x in index]
                fr = H2OFrame._expr(expr=ExprNode("cols", self, index_to_drop), cache=self._ex._cache)
                fr._ex._cache.ncols -= len(index_to_drop)
            ...
            #If index is a string array, i.e., ["C1", "C2"]
                ...
                for i in range(len(index)):
                    if is_type(index[i], str):
                        index_to_drop[i] = self.names.index(index_to_drop[i]) + 1
                index_to_drop = [-x for x in index]
                fr = H2OFrame._expr(expr=ExprNode("cols", self, index_to_drop), cache=self._ex._cache)
                fr._ex._cache.ncols -= len(index_to_drop)
                fr._ex._cache.names[:] = [i for i in self.names]
